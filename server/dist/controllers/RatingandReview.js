...
const createRating = async (req, res) => {
    try {
        ...
        const alreadyReviewed = await RatingandReview_1.default.findOne({
            user: userId,
            course: courseId,
        });
        if (alreadyReviewed) {
        ...
        // Create a new rating and review
        const ratingReview = await RatingandReview_1.default.create({
            rating,
            review,
            course: courseId,
            user: userId,
...
const getAllRatingReview = async (req, res) => {
    try {
        const allReviews = await RatingandReview_1.default.find({})
            .sort({ rating: "desc" })
            .populate({
            path: "user",
